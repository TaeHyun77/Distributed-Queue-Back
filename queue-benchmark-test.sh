...
#
# 측정 항목:
#   1. HTTP 응답 시간 — Redis 중복 확인(ZSCORE×2) + Kafka produce(acks=all)
#   2. 전체 소비 완료 시간 — 첫 요청 → 마지막 메시지 Redis 삽입
#   3. E2E 지연 — 개별 요청: 등록 → Kafka consume → enqueue-or-allow.lua → Redis 삽입
#
# 테스트 흐름:
#   1. 웜업 (50명 등록 → 20초 안정화 → FLUSHALL)
#   2. RPS별 2회 반복 테스트 (FLUSHALL → 본 테스트 → 소비 대기 → 결과 수집)
#   3. 2회 평균 결과 비교표 출력
#
# 사용법:
#   ./benchmark_test.sh                     → 기본 (RPS: 100 300 500 700 1000)
#   ./benchmark_test.sh "100 200 300 400"   → 커스텀 RPS 단계
#
# 사전 조건:
...
echo "  ${WARMUP_STABILIZE}초 안정화 대기 중... (Kafka 소비 + JIT 완료)"
...
for RATE in "${RPS_LEVELS[@]}"; do
    ...
    for ((run=0; run<RUNS; run++)); do
        ...
        # E2E 지연 파싱 (min/avg/max/p95)
        E2E_ALL=""
        for app in "${APPS[@]}"; do
            E2E_ALL+=" $(docker logs --since "$START" "$app" 2>&1 \
                | grep 'E2E completed' \
                | sed -n 's/.*duration=\([0-9]*\)ms.*/\1/p' \
                | tr -d '\r')"
        done

        read E2E_MIN E2E_AVG E2E_MAX E2E_P95 <<< $(calc_e2e_stats "$E2E_ALL")

        R_E2E_MIN[$run]=${E2E_MIN:-0}
        R_E2E_AVG[$run]=${E2E_AVG:-0}
        R_E2E_MAX[$run]=${E2E_MAX:-0}
        R_E2E_P95[$run]=${E2E_P95:-0}

        echo "      E2E: min=${E2E_MIN}ms  avg=${E2E_AVG}ms  p95=${E2E_P95}ms  max=${E2E_MAX}ms"
