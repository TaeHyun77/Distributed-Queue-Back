...
2. 서버는 requestKey를 Redis에 저장하여 중복 요청을 검증합니다. SET NX( SetIfAbsent ) 명령어를 사용해 동일한 requestKey가 이미 존재하면 false를 반환하여 중복 요청이 차단되며, 키가 없는 경우( 최초 요청 )에만 TTL 5분으로 저장 후 정상 요청으로 처리하여 queueType, userId, timestamp 정보를 Kafka로 produce합니다.
