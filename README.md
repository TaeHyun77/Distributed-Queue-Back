...
1. 클라이언트가 서버에 어느 대기열인지를 나타내는 값인 queueType, 사용자의 ID인 userId 정보를 포함하여 대기열 등록 요청을 보내며, 이때 서버는 클라이언트와 SSE 연결을 맺습니다.
...
3. 대기열은 대기큐와 허용큐로 구성되며, 사용자가 두 큐 중 어느 곳에도 존재하지 않을 경우에만 대기큐에 등록됩니다.
    ...
    이후 스케줄러에 의해 일정 인원씩 허용큐로 이동합니다.
    
4. 사용자의 대기열 등록이 완료되면, 서버는 해당 queueType을 Kafka로 발행하며, 이후 Kafka 메시지가 소비될 때 이전에 구독한 Redis 채널로 발행하도록 하여 다른 서버 인스턴스들에 전파됩니다.

5. Redis 채널을 구독 중인 모든 서버 인스턴스는 해당 queueType 이벤트를 수신하게 됩니다.
    
    이를 감지한 서버는 해당 queueType에 속한 각 사용자들의 상태 정보를 SSE를 통해 클라이언트에게 전송합니다.<br><br>
