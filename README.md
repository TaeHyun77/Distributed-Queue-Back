...
이 프로젝트는 기존의 예약 시스템과 대기열 시스템을 개선하여 하나로 통합한 프로젝트입니다.
...
분산 환경에서 발생하는 SSE Sink 동기화 문제를 해결하기 위해 `Redis Pub/Sub`을 활용하여 모든 서버가 동일한 이벤트를 수신, 전달할 수 있도록 하였습니다.<br><br>
...
아울러, 분산 환경에서 여러 서버 간 실시간 SSE Sink 데이터를 일관되게 동기화하기 위해 Redis Pub/Sub을 도입하였으며, 이를 통해 사용자의 대기열 상태와 권한 정보가 모든 서버에 반영되도록 개선하였습니다.

결과적으로 새로운 구조는 데이터베이스 의존성을 제거하여 단일 장애점을 줄이고, 비동기·분산 환경에 적합한 이벤트 전송 및 실시간 동기화를 가능하게 하였습니다.<br><br>
...
하지만, 대기열 시스템은 짧은 시간에 대량의 트래픽이 집중되므로 지속적인 요청이 발생하는 환경에서와 달리 keepalive 설정은 큰 효과를 주지 하며 오히려 자원 낭비로 이어질 수 있습니다. 
