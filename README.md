...
또한, 시스템을 단일 서버에서 분산 서버 구조로 확장하면서 가용성을 높였고, 확장함에 따라 발생하는 서버 간 SSE 전달의 일관성 오류, 스케줄링 중복 등의 문제들을 해결하였습니다.<br><br>
...
1. 대기열 등록 요청이 들어오면, 클라이언트는 어떤 대기열인지를 나타내는 queueType, 사용자 ID인 userId, 멱등키인 idempotencyKey 값을 서버로 전달합니다.
...
3. 여러 서버는 동일한 consumer group으로 구성되어 있기 때문에, Kafka 메시지는 한 서버에서만 consume 하며, Redis Sorted Set 자료구조로 이루어진 대기열에 userId를 key로, timestamp를 score로 저장하여, 자동으로 대기열 순서가 정렬되도록 합니다.
